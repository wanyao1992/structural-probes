    ...
    def forward(self, batch):
        ...
        for i in range(batch_seq_len):
            if i == 0:
                ...
                backward_aggregate[:, batch_seq_len - i - 1, :] = (backward_aggregate[:, batch_seq_len - i,
                                                                   :] * decay_constant) + batch[:,
                                                                                          batch_seq_len - i - 1, :]
                forward_normalization_tensor[i] = forward_normalization_tensor[i - 1] * decay_constant + 1
                backward_normalization_tensor[batch_seq_len - i - 1] = backward_normalization_tensor[
                                                                           batch_seq_len - i] * decay_constant + 1
